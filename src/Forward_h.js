...
function Forward_h() {
    ...
    const trap = () => {
        ...
        if (d === 1) {
            ...
            ans = (f(x + (2 * h)) - 2 * f(x + h) + f(x)) / Math.pow(h, 2)
            ans2 = derivative(derivative(fx, 'x'), 'x').evaluate({x});
         } else if (d === 3) {
            ans = (f(x + (3 * h)) - 3 * f(x + (2 * h)) + 3 * f(x + h) - f(x)) / Math.pow(h, 3)
            ans2 = derivative(derivative(derivative(fx, 'x'), 'x'), 'x').evaluate({x});
        } else if (d === 4) {
            ans = (f(x + (4 * h)) - 4 * f(x + (3 * h)) + 6 * f(x + (2 * h)) - 4 * f(x + h) + f(x)) / Math.pow(h, 4)
        ...
        return(
                ...
                <div className = "App">
                    ...
                    <div className = "up-extext">
                        ...
                        <h1> Forward Divided Difference (h)</h1>     
